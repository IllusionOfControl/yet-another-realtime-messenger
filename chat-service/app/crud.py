...
async def get_or_create_dm(db: AsyncSession, user_a: uuid.UUID, user_b: uuid.UUID) -> Chat:
    stmt = (
        select(Chat)
        .join(ChatMember)
...
async def get_member(db: AsyncSession, chat_id: uuid.UUID, user_id: uuid.UUID) -> Optional[ChatMember]:
