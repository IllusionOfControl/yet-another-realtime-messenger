...
async def test_full_auth_flow(client, db_session):
    ...
    res = await db_session.execute(select(UserLocalAuth).where(UserLocalAuth.email == user_data["email"]))
    local_auth = res.scalar_one()
    verification_code = local_auth.verification_code

    verify_resp = await client.post("/api/v1/auth/verify-email", params={"code": verification_code})
    ...
    login_username_resp = await client.post("/api/v1/auth/login", json={
