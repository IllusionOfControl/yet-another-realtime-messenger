...
async def register_user(
    ...
    await create_local_user(
        db, user_profile.id, user_create, hashed_password, verification_code,
    )

    # TODO: Отправить Kafka событие для Notification Service, чтобы тот выслал письмо с кодом

    return SuccessResponse(message="User registered successfully. Please verify your email.")
...
async def login_for_access_token(
    ...
    user_session = await create_user_session(db, local_auth.user_id, access_jti, refresh_jti, issued_at, refresh_token_expires_at)
...
async def refresh_access_token(
    ...
    await update_session_after_refresh(
        db, user_session, new_access_jti, new_refresh_jti, issued_at, refresh_token_expires_at
    )
